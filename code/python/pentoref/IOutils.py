...
def get_data_from_xml(xmlpath):
    ...
    for folder in sorted(listdir(xmlpath)):
        for file_name in sorted(listdir(join(xmlpath, folder))):
            if not isdir(file_name):
                name, ext = splitext(file_name)
                if ext.lower() == '.txt':
                    if 'final-selected' in name:
                        tiles[name[:-15]] = (open(join(xmlpath,
                                                       folder,
                                                       file_name)).read())
                    elif 'landmark' in name:
                        landmarks[name[:-9]] = (open(join(xmlpath,
                                                          folder,
                                                          file_name)).read())
                elif ext.lower() == '.xml' and 's' not in name:
                    try:
                        xml = "".join([line for line in open(join(xmlpath,
                                                                  folder,
                                                                  file_name))])
                        xml = xml.encode("utf-8").decode("utf-8")
                        # reader = StringIO(xml)
                        # scene_descriptions[name] = lxml.etree.parse(reader)
                        scene_descriptions[name] = xml
                    except XMLSyntaxError:
                        print "Error parsing file_name!"
                        print join(xmlpath, folder, file_name)
                        return None, None, None
    print(str(len(tiles.keys())+len(landmarks.keys())) +
          ' text files read. '+str(len(scene_descriptions)) +
          ' XML files parsed.')
    return tiles, landmarks, scene_descriptions


def create_data_from_transcriptions_and_xml_data_PentoCV(corpus,
                                                     transcr_path,
                                                     xmlpath):
    print corpus
    # get data from the paths
    transcr_format = get_transcription_or_annotation_input_format(tgpath)
    textgrids = get_data_from_transcriptions(tgpath, transcr_format)
            ...
            for key in scene_descriptions.keys():
                ...
                if rx == run:
                    ...
                    for piece in scenerep.findall('object'):
                        ...
                        try:
                            current_piece['gameID'] = run+'_'+find(textgrids[run],(time,time,''))['Part'][2]
                        except:
                ...
                try:
                    if corpus == 'TAKE':
                        for piece in scene_descriptions[current_ref['gameID']].findall('.//piece'):
                            ...
                            current_piece['isMirrored'] = piece.find('posture').get('isMirrored')
                            current_piece['gridPosition'] = piece.find('start-field').text
                            scenes.append(current_piece)
                    else:
                        scenerep = parse(StringIO(scene_descriptions[current_ref['gameID'].replace('.','_')]))
                        for piece in scenerep.findall('timestamp/object'):
                            current_piece = dict()
                            current_piece['gameID'] = current_ref['gameID']
                            current_piece['pieceID'] = piece.get('id')
                            current_piece['isLandmark'] = piece.get('isLandmark')
                    ...
                    log.write(str(start)+' seconds: No usable scene information found for gameID "'+current_ref['gameID']+'". No scene has been included.\n')
        log.write('\nutterances\n\n')
        ...
        for speaker in speakers:
            ...
            for interval in textgrids[run][speaker+'-utts']:
                ...
                if text != 'p':
                    ...
                    if corpus == 'PentoCV':
                        try:
                            episode_lastfind = find(textgrids[run],interval)['Part'][2]
                        except:
                    ...
                    if corpus == 'PENTOCV':
                        try:
                            current_utt['gameID'] = run+'_'+find(textgrids[run],interval)['Part'][2]
                        except:
                    ...
                    if len(speakers) > 1:
                        ...
                        try:
                            current_utt['role'] = {role.split()[0]:role.split()[1] for role in find(textgrids[run],interval)['Roles'][2].split(', ')}[speaker]
                            previous_role = current_utt['role']
                        except:
                            current_utt['role'] = previous_role
                    if corpus == 'TAKE':
                        current_utt['dialogue-act'] = find(textgrids[run],interval)[speaker+'-dialogue-acts'][2]
                ...
                for interval in textgrids[run][speaker+'-words']:
                    if len(interval) > 0:
                        ...
                        try:
                            ...
                            log.write(str(start)+' seconds: Lemmatization has failed for interval "'+text+'".\n')
                        try:
                            episode_lastfind = find(textgrids[run],interval)['Episode'][2]
                        except:
                        ...
                        try:
                            utt_lastfind = find(textgrids[run],interval)[speaker+'-utts'][2]
                        except:
            ...
            for hand in ['lh','rh']:
                for interval in textgrids[run][hand]:
                    ...
                    try:
                        ...
                        log.write(str(start)+' seconds: The action "'+text+'" could not be read correctly.\n')
                    try:
                        current_action['gameID'] = run+'_'+find(textgrids[run],interval)['Part'][2]
                        actions.append(current_action)
                    except:
                        log.write(str(start)+' seconds: No gameID found for interval "'+text+'". No action has been included.\n')
...
def create_data_from_transcriptions_and_xml_data_new(corpus,
                                                     transcr_path,
                                                     xmlpath):
    ...
    for run in textgrids.keys():
        ...
        if corpus == 'PENTOCV':
            for key in scene_descriptions.keys():
                ...
                time = float(skey[1])/1000
                if rx == run:
                    reader = StringIO(scene_descriptions[key])
                    scene_rep = lxml.etree.parse(reader)
                    for piece in scene_rep.findall('object'):
                        current_piece = dict()
                        current_piece['timestampID'] = time
                        current_piece['pieceID'] = piece.get('id')
                        current_piece['isLandmark'] = piece.get('isLandmark')
            ...
            for interval in textgrids[run]['Episode']:
                ...
                try:
                    if corpus == 'TAKE':
                        reader = StringIO(scene_descriptions[current_ref['gameID']])
                        scene_rep =  lxml.etree.parse(reader)
                        for piece in scene_rep.\
                                    findall('.//piece'):
                            current_piece = dict()
                            current_piece['gameID'] = current_ref['gameID']
                            current_piece['pieceID'] = piece.get('id')
                        ...
                        reader = StringIO(scene_descriptions[ current_ref['gameID']
                                                .replace('.', '_')])
                        scene_rep = lxml.etree.parse(reader)
                        for piece in scene_rep.findall('timestamp/object'):
                            current_piece = dict()
                            current_piece['gameID'] = current_ref['gameID']
                            current_piece['pieceID'] = piece.get('id')
...
def create_data_from_transcriptions_and_xml_data(corpus,tgpath,xmlpath):
    ...
    for run in textgrids.keys():
        ...
        for interval in textgrids[run]['Episode']:
            ...
            try:
                if corpus == 'TAKE':
                    reader = StringIO(scene_descriptions[
                                                        current_ref['gameID']])
                    scene_rep = lxml.etree.parse(reader)
                    for piece in scene_rep.findall('.//piece'):
                        current_piece = dict()
                        current_piece['gameID'] = current_ref['gameID']
                        current_piece['pieceID'] = piece.get('id')
                        current_piece['shape'] = piece.get('type')
